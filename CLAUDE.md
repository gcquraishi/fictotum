...
- **Illustrations in production (portraits-and-fiction M1)**: 818/1,008 illustrations uploaded to Vercel Blob and linked to Neo4j nodes (480 this session + 338 prior). Search results now show portrait thumbnails via new SearchThumbnail component. Fixed upload-and-link.ts blob options (`addRandomSuffix: false`, `allowOverwrite: true`). Added `image_url` to `searchFigures()` query. Fixed pre-existing suggest-eras TS error blocking Vercel builds. 190 remaining illustrations blocked by Vercel Blob free tier operation limit (2,000 ops/month).
