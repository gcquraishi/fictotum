"""
Fictotum: Batch 5 Ingestion
Ingests deduplicated batch 5 data into Neo4j.
Focus: American Revolution, Victorian London, The Shadow History
"""
import os
import sys
import json
...
def infer_media_type(title: str) -> str:
    ...
    # Game indicators
    if 'assassin' in title_lower or 'creed' in title_lower:
        return 'Game'

    # TV Series indicators
    if 'turn' in title_lower or 'penny dreadful' in title_lower or 'alienist' in title_lower:
        return 'TVSeries'

    # Film indicators
    if 'vampire hunter' in title_lower:
        return 'Film'

    # Book is default (Adventures of Sherlock Holmes)
    return 'Book'

class Batch5Ingestor:
    """Handles ingestion of Batch 5 data into Neo4j."""
    ...
    def ingest_historical_figures(self, figures: list) -> int:
        ...
        with self.driver.session() as session:
            for figure in figures:
                try:
                    # Handle null birth/death years for Jack the Ripper
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch5_deduplicated.json"
    with open(data_path, 'r') as f:
        data = json.load(f)

    print("=" * 70)
    print("Fictotum: Batch 5 Ingestion")
    print("=" * 70)
    ...
    try:
        ingestor = Batch5Ingestor(uri, username, password)
