"""
Fictotum: Batch 6 Ingestion
Ingests deduplicated batch 6 data into Neo4j.
Focus: World War II, Bletchley Park, The Manhattan Project, The Intelligence Nexus
"""
import os
import sys
import json
...
def infer_media_type(title: str) -> str:
    ...
    # Film indicators
    if any(x in title_lower for x in ['imitation game', 'oppenheimer', 'inglourious', 'saving private ryan']):
        return 'Film'

    # TV Series indicators
    if 'band of brothers' in title_lower:
        return 'TVSeries'

    # Book is default (Casino Royale, The Man in the High Castle)
    return 'Book'

class Batch6Ingestor:
    """Handles ingestion of Batch 6 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch6_deduplicated.json"
    with open(data_path, 'r') as f:
        data = json.load(f)

    print("=" * 70)
    print("Fictotum: Batch 6 Ingestion")
    print("=" * 70)
    ...
    try:
        ingestor = Batch6Ingestor(uri, username, password)
