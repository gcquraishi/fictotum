"""
Fictotum: Batch 4 Ingestion
Ingests deduplicated batch 4 data into Neo4j.
Focus: The Terror, The Crusades (Levant), Saxon/Danish Collision
"""
import os
import sys
import json
...
def infer_media_type(title: str) -> str:
    ...
    # Film indicators
    if 'kingdom of heaven' in title_lower:
        return 'Film'
    ...
    if 'last kingdom' in title_lower:
        return 'TVSeries'

    # Book is default
    return 'Book'

class Batch4Ingestor:
    """Handles ingestion of Batch 4 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch4_deduplicated.json"
    with open(data_path, 'r') as f:
        data = json.load(f)

    print("=" * 70)
    print("Fictotum: Batch 4 Ingestion")
    print("=" * 70)
    ...
    try:
        ingestor = Batch4Ingestor(uri, username, password)
