...
def infer_media_type(title: str) -> str:
    ...
    # Film indicators
    if any(x in title_lower for x in ['alexander', 'troy']):
        return 'Film'

    # Game indicators
    if 'assassin' in title_lower or 'odyssey' in title_lower:
        return 'Game'

    # Graphic Novel indicators
    if '300' in title_lower and 'graphic' in title_lower:
        return 'GraphicNovel'

    # Book is default (Gates of Fire, Song of Achilles, The Iliad)
    return 'Book'

class Batch8Ingestor:
    """Handles ingestion of Batch 8 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch8_deduplicated.json"
    ...
    try:
        ingestor = Batch8Ingestor(uri, username, password)
