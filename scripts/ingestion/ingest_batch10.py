...
def infer_media_type(title: str) -> str:
    ...
    # Game indicators
    if any(x in title_lower for x in ['dynasty warriors', 'total war']):
        return 'Game'

    # Film indicators
    if any(x in title_lower for x in ['bladesman', 'lost bladesman']):
        return 'Film'

    # Manga indicators
    if any(x in title_lower for x in ['ravages of time', 'sōten kōro', 'beyond the heavens']):
        return 'Manga'

    # TV Series indicators
    if 'three kingdoms' in title_lower and 'series' in title_lower:
        return 'TVSeries'

    # Book is default
    return 'Book'

class Batch10Ingestor:
    """Handles ingestion of Batch 10 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch10_deduplicated.json"
    ...
    try:
        ingestor = Batch10Ingestor(uri, username, password)
