"""
Fictotum: Batch 7 Ingestion
Ingests deduplicated batch 7 data into Neo4j.
Focus: The Cold War, Espionage, The Space Race, The Civil Rights Era
"""
import os
import sys
import json
...
def infer_media_type(title: str) -> str:
    ...
    # Film indicators
    if any(x in title_lower for x in ['dr. strangelove', 'bridge of spies', 'hidden figures']):
        return 'Film'

    # TV Series indicators
    if 'mad men' in title_lower:
        return 'TVSeries'

    # Book is default (The Right Stuff, Watchmen)
    return 'Book'

class Batch7Ingestor:
    """Handles ingestion of Batch 7 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch7_deduplicated.json"
    with open(data_path, 'r') as f:
        data = json.load(f)

    print("=" * 70)
    print("Fictotum: Batch 7 Ingestion")
    print("=" * 70)
    ...
    try:
        ingestor = Batch7Ingestor(uri, username, password)
