"""
Fictotum: Batch 3 Ingestion
Ingests deduplicated batch 3 data into Neo4j.
"""
import os
import sys
import json
...
def infer_media_type(title: str) -> str:
    ...
    if any(x in title_lower for x in ['hbo', 'series', 'tudors', 'boardwalk', 'empire', 'gilded age']):
        return 'TVSeries'

    # Film indicators
    if any(x in title_lower for x in ['untouchables', 'red cliff', 'gone with the wind']):
        return 'Film'

    # Game indicators
    if any(x in title_lower for x in ['basara', 'assassin', 'creed']):
        return 'Game'

    # Musical indicators
    if 'hamilton' in title_lower:
        return 'Musical'

    # Book is default
    return 'Book'

class Batch3Ingestor:
    """Handles ingestion of Batch 3 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch3_deduplicated.json"
    with open(data_path, 'r') as f:
        data = json.load(f)

    print("=" * 70)
    print("Fictotum: Batch 3 Ingestion")
    print("=" * 70)
    ...
    try:
        ingestor = Batch3Ingestor(uri, username, password)
