...
def infer_media_type(title: str) -> str:
    ...
    # Film indicators
    if any(x in title_lower for x in ['kagemusha', 'ran']):
        return 'Film'

    # Game indicators
    if any(x in title_lower for x in ['sekiro', 'nioh', 'onimusha']):
        return 'Game'

    # Manga/Anime indicators
    if any(x in title_lower for x in ['vagabond', 'inuyasha']):
        return 'Manga'

    # TV Series indicators
    if 'blue eye samurai' in title_lower:
        return 'TVSeries'

    # Book is default
    return 'Book'

class Batch9Ingestor:
    """Handles ingestion of Batch 9 data into Neo4j."""
...
def main():
    ...
    data_path = Path(__file__).parent.parent.parent / "data" / "global_mvp_batch9_deduplicated.json"
    ...
    try:
        ingestor = Batch9Ingestor(uri, username, password)
